...
- **Phase 11.4 / 11.5 / 11.6 (ADRs 0117 / 0118 / 0119)** — Remaining
  FE-rebuild sub-phases. 11.0–11.3 SHIPPED (ADR 0116). Stack + design
  tokens + 12 component primitives + onboarding + worker + citizen
  surfaces are live; `/app/` serves the SPA via the existing API
  server. Remaining: 11.4 verifier adaptation (MFI bundle reader,
  Trust Passport public view, ~2 days), 11.5 labs catch-all (SLM
  install wire to Phase 9.0b endpoints, federated rounds, OCR, voice
  settings, ~2 days), 11.6 polish + Playwright end-to-end investor
  demo smoke (~1 day). Total remaining: ~5 days.
- **Phase 11 — DEPRECATED PROPOSAL (ADR 0115)** — Original Phase 11 rebuild proposal.
